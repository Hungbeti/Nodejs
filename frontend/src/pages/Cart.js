...
const Cart = () => {
  ...
  const validateCouponAction = async (code, currentCartItems, currentSelectedIds) => {
    ...
    const selectedCartItems = currentCartItems.filter(item => currentSelectedIds.includes(item._id));

    if (selectedCartItems.length === 0) return 0;

    // --- SỬA LẠI ĐOẠN NÀY ---
    const itemsPayload = selectedCartItems.map(item => {
      // Lấy ID gốc an toàn (Logic tương tự như handleCheckout)
      const rawId = item.product._id || item.product;
      const cleanProductId = rawId ? rawId.toString().split('-')[0] : rawId;

      return {
          product: cleanProductId, // Gửi ID sạch để Backend tìm được sản phẩm và check Category
    ...
    try {
      ...
      // Nếu lỗi 400, có thể do backend trả về message lỗi cụ thể, hiển thị ra để debug
