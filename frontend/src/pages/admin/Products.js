...
const AdminProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      // Thay đổi limit và page dựa trên state
      ...
      // Cập nhật tổng số trang từ server trả về
  ...
  useEffect(() => { if (tab === 'products') fetchProducts(); }, [tab, currentPage, productSearch, productCategoryFilter]);
  useEffect(() => { if (tab === 'categories') fetchCategories(); }, [tab]);
  useEffect(() => { if (tab === 'brands') fetchBrands(); }, [tab]);
            ...
            <div className="card p-3 shadow-sm">
              <div className="row g-2">
                ...
                <div className="col-md-3 text-end">
                  {/* Nút thêm mới chuyển xuống đây cho gọn hoặc giữ nguyên vị trí cũ tuỳ bạn */}
                  <button className="btn btn-success w-100" onClick={() => { resetProduct(); setShowProductModal(true); }}>
                ...
                <tbody>
                  {brands
                    .filter(b => {
                      ...
                      // Nếu brandCategoryFilter rỗng thì lấy hết, nếu có thì so sánh ID
                      const matchCategory = brandCategoryFilter 
                        ? (b.category && b.category._id === brandCategoryFilter) 
                        : true;
