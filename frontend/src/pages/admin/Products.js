...
const AdminProducts = () => {
  ...
  // State cho phân trang (ví dụ)
  const [productPage, setProductPage] = useState(1);
  const [productTotalPages, setProductTotalPages] = useState(1);

  /* ---------- FETCH ---------- */

  // Hàm helper để tạo query string
  ...
  // Tải lại dữ liệu cần thiết khi chuyển tab
  useEffect(() => {
    if (tab === 'products') {
      fetchCategories(); // Cần cho form
    }
    if (tab === 'brands') {
      fetchCategories(); // Cần cho form
    }
  ...
  // Tải thương hiệu cho form sản phẩm khi chọn danh mục
  ...
  const editProduct = (p) => {
    ...
    setEditingProduct(p);
    if (p.category?._id) {
      handleProductCategoryChange(p.category._id);
    }
  };
